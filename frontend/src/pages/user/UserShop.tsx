...
const SHOP_ITEMS = [
    { id: 1, type: 'consumable', name: '神奇營養液', description: '下次挑戰經驗值加倍！', price: 50, icon: '🧪', levelRequired: 1 },
    { id: 2, type: 'decoration', name: '時尚草帽', description: '為你的寵物添購一頂可愛的草帽。', price: 150, icon: '👒', levelRequired: 1 },
    { id: 3, type: 'consumable', name: '豐收肥料', description: '立即獲得 100 顆種子！', price: 200, icon: '🌾', levelRequired: 1 },
    { id: 4, type: 'decoration', name: '酷炫墨鏡', description: '為你的寵物戴上一副時尚墨鏡。', price: 200, icon: '🕶️', levelRequired: 3 },
    { id: 5, type: 'decoration', name: '花紋圍巾', description: '柔軟溫暖的花紋圍巾。', price: 250, icon: '🧣', levelRequired: 3 },
    { id: 6, type: 'decoration', name: '植物學家之冠', description: '屬於真正植物學家的榮耀之冠。', price: 500, icon: '👑', levelRequired: 5 },
];
...
export function UserShop(){
    ...
    const confirmPurchase = () => {
        if (purchaseModal) {
            // TODO: Call backend reducer to update seeds and add item to inventory
        ...
        <div className="shop-container">
            ...
            <div className="shop-items-list">
                {filteredItems.map(item => {
                    ...
                    const isAvailable = canAfford && isUnlocked;
                    ...
                    return (
                        <div key={item.id} className="shop-item">
                            ...
                            <div className="item-right">
                                ...
                                <button
                                    className={`buy-btn ${isAvailable ? 'active' : isUnlocked ? 'owned' : 'locked'}`}
                                    onClick={() => handlePurchase(item)}
                                    disabled={!isAvailable}
                                >
                                    {!isUnlocked ? '上鎖' : isAvailable ? '購買' : '不足'}
                                </button>
                            </div>
                        </div>
                    );
